<script>
	...
	function formatForSearch(value) {
		...
		if (Array.isArray(value)) {
			...
			return Object.entries(value).map(([key, val]) => `${key}: ${formatForSearch(val)}`).join(' ');
		}
...
<div bind:this={containerRef} class="table-container" style="height: {tableHeight}px">
	<div class="sticky-header">
		{#each Object.keys(columns) as key}
			<div class="header-cell" style="width: {columnWidths[key] || defaultColumnWidth}px" title={key} on:click={() => handleSort(key)} role="button" tabindex="0" on:keypress={(e) => { if (e.key === 'Enter' || e.key === ' ') handleSort(key); }} >
				{key}
				{#if sortKey === key}
					{sortOrder === 'asc' ? ' ▲' : ' ▼'}
				{/if}
				<div class="resize-handle" on:mousedown={(event) => handleMouseDown(event, key)} role="separator" aria-label="Resize column" />
			</div>
		{/each}
	</div>
...
<style>
    .table-container { overflow-y: auto; overflow-x: auto; position: relative; border: 1px solid #ccc; }
    .sticky-header { position: sticky; top: 0; background-color: #f4f4f4; z-index: 2; display: flex; width: fit-content; }
    .header-cell, .cell { position: relative; padding: 8px; border: 1px solid #ddd; text-align: left; white-space: nowrap; width: 200px; overflow: hidden; text-overflow: ellipsis; box-sizing: border-box; display: flex; align-items: center;}
    .resize-handle { position: absolute; right: 0; top: 0; width: 5px; height: 100%; cursor: ew-resize; background-color: transparent; }
    .row { display: flex; position: absolute; width: fit-content; }
    .search-container { display: flex; justify-content: flex-end; margin-bottom: 10px; }
    :global(mark) { background-color: rgb(254, 254, 0); color: inherit; padding: 0; font-weight: bold; }
</style>